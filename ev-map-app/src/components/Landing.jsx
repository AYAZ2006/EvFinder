...
useEffect(()=>{
  if(inView1)control1.start({opacity:1,y:0})
  },[inView1])
useEffect(() => {
  if(inView2)control2.start({opacity:1,y:0})
  },[inView2])
useEffect(()=>{
  if(inView3)control3.start({opacity:1,y:0})
  },[inView3])
  return(
    <div className="w-screen min-h-screen overflow-hidden bg-[url('/image.png')] bg-center bg-cover text-white">
      ...
      <Parallax pages={3} className="overflow-y-scroll scrollbar-hidden">
        <ParallaxLayer offset={0} speed={0} className="flex flex-col items-center gap-8 px-4 py-8">
          <motion.div ref={ref1} initial={{ opacity: 100, y: 50 }} animate={control1} transition={{ duration: 0.8, delay: 0.2 }} className="bg-gradient-to-r from-white via-gray-50 to-white shadow-2xl rounded-3xl w-full max-w-6xl h-fit transition-transform hover:scale-[1.02] hover:shadow-blue-100 flex items-center justify-center p-6 md:p-10">
            <div className="flex flex-col md:flex-row items-start gap-6">
              <img src="/bike.png" className="max-h-90 w-auto object-contain rounded-xl md:mr-6"></img>
              <div className="bg-blue-100 text-blue-600 rounded-full p-3 self-start">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z"/>
                </svg>
              </div>
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold text-blue-800 mb-2">{infoSections[0].title}</h2>
                {infoSections[0].paragraphs.map((text, i) => (
        ...
        <ParallaxLayer offset={1} speed={0} className="flex flex-col items-center gap-8 px-4 py-8">
          <motion.div ref={ref2} initial={{ opacity: 0, y: 50 }} animate={control2} transition={{ duration: 0.8, delay: 0.1 }} className="bg-gradient-to-r from-white via-gray-50 to-white shadow-2xl rounded-3xl w-full max-w-6xl h-fit transition-transform hover:scale-[1.02] hover:shadow-blue-100 flex items-center justify-center p-6 md:p-10">
            <div className="flex flex-col md:flex-row items-start gap-6">
              <img src="/car.png" className="max-h-80 w-auto object-contain rounded-xl md:mr-6 lg:mr-9"/>
              <div className="bg-blue-100 text-blue-600 rounded-full p-3 self-start">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 20a8 8 0 100-16 8 8 0 000 16z"/>
                </svg>
              ...
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold text-blue-800 mb-2">{infoSections[1].title}</h2>
                {infoSections[1].paragraphs.map((text, i) => (
                  <p key={i} className={`text-gray-700 leading-relaxed ${i !== 0 ? 'mt-4' : ''}`}>{text}</p>
                ))}
              </div>
            </div>
        ...
        <ParallaxLayer offset={2} speed={0} className="flex flex-col items-center gap-8 px-4 py-8">
          <motion.div ref={ref3} initial={{ opacity: 0, y: 50 }} animate={control3} transition={{ duration: 0.8, delay: 0.2 }} className="bg-gradient-to-r from-white via-gray-50 to-white shadow-2xl rounded-3xl w-full max-w-6xl h-fit transition-transform hover:scale-[1.02] hover:shadow-blue-100 flex items-center justify-center p-6 md:p-10">
            <div className="flex flex-col md:flex-row items-start gap-4">
              <img src="/auto1.png" className="max-h-80 w-auto object-contain rounded-xl md:mr-6 lg:mr-9 lg:mt-6"/>
